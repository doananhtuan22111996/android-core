 ...
 */
abstract class PagingByLocalDataSource<RequestType : Any, ResultType : Any>(private val dispatcher: CoroutineDispatcher = Dispatchers.IO) : PagingSource<Int, ResultType>() {

    override fun getRefreshKey(state: PagingState<Int, ResultType>): Int? = state.anchorPosition?.let { anchorPosition ->
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, ResultType> = withContext(dispatcher) {
        try {
            ...
            Timber.e("PagingByLocalDataSource Failure: ${e.message}")
            LoadResult.Error(
                Throwable(
